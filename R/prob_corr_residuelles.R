...
prob_corr_residuelles <- function(R, AB, N) {
  ...
  # Paires dans l'ordre du triangle supérieur (colonne-majeure)
  ...
  for (s in seq_len(np)) {
    ...
    # Carrés des saturations estimées : x² = rAX·rBX / rAB
    # Si x² ou y² ≤ 0 (signes incohérents), la saturation est essentiellement
    # nulle : on substitue (.001)² pour éviter les NA tout en conservant xy ≈ 0.
    x2 <- rAX * rBX / rAB
    y2 <- rAY * rBY / rAB
    x2 <- max(x2, 1e-6)
    y2 <- max(y2, 1e-6)

    # Produit des saturations (avec signe cohérent)
    ...
    # ∂atanh(xy)/∂rAX = xy / (2·rAX·(1−xy²))  ;  ∂/∂rAB = −xy / (rAB·(1−xy²))
    c2xy  <- 1 - xy^2
    c2rXY <- 1 - rXY^2
    g <- c(
       1 / c2rXY,
      ...
      -xy / (2 * rAY * c2xy),
      -xy / (2 * rBY * c2xy),
       xy / (rAB * c2xy)        # signe positif : -(−xy/rAB)
    )

    # Indices des 6 corrélations dans R
    idx6 <- list(c(ix, iy), c(ia, ix), c(ib, ix),
                 c(ia, iy), c(ib, iy), c(ia, ib))

    # Matrice de covariance 6×6 des corrélations observées
    Sig <- matrix(0, 6, 6)
    for (m in seq_len(6))
      for (n in m:6) {
